# Chiaki Frontend Rulebook v12

## はじめに

このルールブックは、Next.js + GSAP Playground
を安定して育てながら、将来の別案件にも転用しやすい実装基準をまとめたものです。\
...
App Router を前提に、React Component は **Server Component を基本**
とします。\
...
HTML は、**意味のある構造** と **レイアウトの都合** を分けて考える。\
見た目のためだけに semantic 要素を増やさず、逆に意味のある区画は素直に semantic 要素を使う。

### 標準構造

通常のページでは、次の構造を標準形とする。
...
これは **標準形** であり、すべてのページで強制する唯一の正解ではない。
内容に応じて `article` や `div` を使う構造も許容する。
...
ページ内の **意味のある区画** を表す。
Hero、About、Service、Contact のように、内容や役割の単位として区切れる領域に使う。
セクション単位の `padding-block`、背景切り替え、区切りを担当してよい。
ただし、単なるレイアウト都合や局所的なグルーピングのためだけには使わず、その場合は `div` を使う。

#### `article`

文書として独立性が高い内容に使う。
記事本文、個別エントリー、読み物ページなどに用いる。

#### `div`

意味付けを必要としないレイアウト用の箱として使う。
局所的なグルーピング、補助 UI、装飾ブロック、内部レイアウトの整理には `div` を使う。
...
`container` の内側で、内部レイアウトを担当する層。
`flex` / `grid` / `gap` / columns / alignment などは `inner` 側に持たせる。

### 運用ルール

- `main` は、そのページの主要コンテンツ領域として原則使用する
- `section` は、ページ内の意味のある区画に使う
- 単なるレイアウト調整や局所的なグルーピングには `div` を使う
- 文書として独立性が高い内容には `article` を使う
- `main > section > container > inner` は標準形とする
- ただし、内容に応じて `main > article > container` や `main > div > container` も許容する

### container / inner 運用ルール（v12 更新）

- `container` は global class として定義する
- `inner` は module.css に定義する
- 基本は `container > inner` として分離する
- `container` は横幅制御のみを担当する
- `inner` は内部レイアウトのみを担当する
- class 分離のみを目的とした不要な DOM ネストは作らない
...
- `flex` / `grid` / `gap` を持つ
- レイアウト方向や配置制御を持つ
- セクション内でコンテンツ構造を整理したい
- 今後の拡張を考えて、横幅制御と内部レイアウトを分けた方が明確
...
以下のように、内容が単純で内部レイアウト層を切る意味が薄い場合のみ許容する。
...
- semantic に見せるためだけに `section` や `article` を使わない
- class 分離だけを目的に DOM を増やさない
- `container` に `flex` / `grid` / `gap` を持たせない
- 役割の曖昧なラッパーを慣習的に増やさない
...
- local custom property
  は、構造寸法・計算用・同一ファイル内で複数参照する値に限定する
- 名前を変えるためだけの local token は作らない
- global token を local custom property
  に置き換えてコード量を増やさない
- local custom property
  を作るときは、「そのファイルでまとめて管理する意味があるか」で判断する
...
- `calc()` の基準になる構造値
- left column width / indicator position / media ratio
  のようなページ固有の構造値
...
これらのように、単に global token
を別名にしているだけのものは作らず、最初から global token を直接使う。
...
- `640px`、`900px`、`959px`、`1000px` のような近接値は、原則 shared
  breakpoint に統一する
- `max-width`
  は例外的に「ある幅未満だけを限定的に上書きしたい」場合にのみ使う
- ただし、GSAP などの動きの有効条件は breakpoint 単体ではなく
  capability 判定と分離する
...
- base style は narrow layout として書き、必要な構造だけを `min-width`
  で追加する
...
- 「desktop だから動かす」ではなく、`MEDIA_QUERIES.desktopMotion`
  を満たすときだけ動かす
...
PC 向けのスクロール演出（pin / scrub / horizontal scroll / fullscreen
reveal
など）は、すべての端末で強制的に動かすのではなく、**操作環境とパフォーマンスに応じて制御する**。
...
- component 専用の offset / hover shift / 一時調整値は local custom
  property で持つ
...
- ページ専用カラーは必要最小限にとどめ、まずは global color token
  で組み立てる
- before / after
  など隣接セクションとの背景差が必要な場合でも、可能なら `surface` 系
  token を優先して使う
...
- global color token で十分表現できるのに、ページ専用 color token
  を増やさない

### 運用補足

- color token は、まず solid token（background / foreground / surface
  / border）で組み立てる
- alpha を使う token は、text / control / panel / ui-line / ui-dot
  のように役割単位で管理する
- 透明度の段階は必要最小限にとどめ、近い値のために token
  を細分化しない
- 新しい alpha token
  を追加する前に、既存の近い段階へ統合できないかを確認する
- light / dark
  は数値を機械的に対称にせず、知覚上のコントラストが近づくように調整する
...
- hover スタイルは `@media (hover: hover) and (pointer: fine)`
  の中でのみ書く
- touch 端末では hover を前提にしない
- touch 端末のフィードバックは `:active` を使う
- 選択状態や開閉状態は hover ではなく、React state / `data-state` /
  ARIA 属性で管理する
...
- 見た目変化が知覚しづらい場合は、無理に scale を入れず border /
  background の変化だけでもよい
...
GSAP
の実験ページを増やしながら、後から見返しても分かりやすく、保守しやすく、必要に応じてポートフォリオにも転用できる構成を保つことを目的とします。
...
今は使わないが残しておきたいもの、没案、試作段階のものは `archive`
に移す。
...
Home 用メタデータは `app/experiments/_registry/experiments.ts`
に集約する。
...
experiments の子ページでも、Part 2 の HTML / レイアウト設計に従う。\
`wrapper` は基本構造として使わない。
...
- `main` は experiments ページでも原則使用する
- `section` は意味のある区画がある場合に使う
- 単一構成で十分な場合は `main > div > container > inner` でもよい
- `wrapper` という曖昧なラッパー名は使わない
- root 要素の class 名は役割に応じて `main` / `section` / `panel` などで命名する
...
cleanup を統一しやすく、`scope` によって DOM
探索範囲を限定しやすいため。
...
GSAP / ScrollTrigger / ticker / listener は、`useGSAP` または effect の
`return` で必ず cleanup する。
...
pin / scrub / fullscreen reveal / horizontal translation のような PC
専用演出は、`Desktop Motion Gating` の条件に従って ON / OFF を判定する。
...
Lenis の ON / OFF も、原則として Desktop Motion Gating
と矛盾しない条件で運用する。\
PC 専用の重いスクロール演出を使わない環境では、native scroll
を優先する。
...
Desktop Motion Gating に該当しない環境では、parallax の transform
をリセットし、静的表示に戻す。
...
そのため、ScrollTrigger
による重い演出よりも、一覧としての軽さ、可読性、絞り込みやすさを優先する。
...
- `EXPERIMENTS.filter(...)` 程度の軽い計算には慣習的に `useMemo`
  を使わない
- tag hover のような局所調整値は local custom property でよい
- tag / card / back button のように複数ファイルで繰り返す interaction
  値のみ global token 化を検討する
...
- ただし、本文の見た目統一が必要な場合は `StepParagraph` / `StepNote`
  のような小さな presentational component を使う
...
このファイルは `Chiaki Frontend Rulebook v12` を基準版とする。\
...
spacing / typography / color などの token
を更新する場合は、名前だけでなく値まで本文に反映する。
...
- 更新日: 2026-04-11
- HTML / レイアウト設計を全面的に再整理
- `wrapper` を基本構造および experiments ページ運用から完全削除
- `section` を「見出し必須の独立文脈」ではなく、「ページ内の意味ある区画」として再定義
- `main > section > container > inner` を標準形として整理
- `container` を横幅制御専用、`inner` を内部レイアウト専用として分離
- `container` と `content` の同一要素運用を廃止し、`container > inner` を基本ルールへ変更
- 単一ページ、記事ページ、実験ページにおける構造選択の目安を追加
- full-bleed に関する過度な固定ルールを削除
- Part 2 と Part 3 の競合を解消し、HTML 構造ルールを一本化
...
- token / responsive / motion / interaction
  の関連ルールを近接配置し、重複参照を減らした
- v9 の内容整理に加えて、media query
  運用の改善提案を別紙で扱いやすい形に分離した
- breakpoint と capability 判定の役割分離を、Responsive / Desktop
  Motion Gating / GSAP / Lenis の間で読みやすく再整理
- Rulebook Update Policy の基準 version を v10 に更新

### v9

- 更新日: 2026-04-07
- `globals.css` の最新 token に合わせて color / semantic token
  記述を更新
...
- color token の使い方を整理し、ページ専用 color token
  の増殖を避ける方針を追加
- GSAP 実装整理ルールを追加
...
- GSAP / Lenis / parallax の運用方針に Desktop Motion Gating
  との整合ルールを追加
...
- app 全体で使用する breakpoint を `48rem / 64rem / 75rem / 90rem`
  に統一
...
- 更新日: 2026-03-25
- Part 構成を
  `Core Principles / Shared Implementation Rules / Current Project Rules`
  に再編
- 重複していた内容を整理し、判断基準と実装ルールを分離
- `Next.js / React`、`レイアウト`、`CSS`、`token`、`アクセシビリティ`
  の順に再配置
...
- radius / motion / blur / control / panel / ui-line / ui-dot token
  を明記
