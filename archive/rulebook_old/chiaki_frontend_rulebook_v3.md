...
通常の UI では `hover` / `pointer` を基準にし、`any-hover` / `any-pointer` は hybrid device 対応が本当に必要な場合のみ検討します。
...
このファイルは `Chiaki Frontend Rulebook v2` を基準版とします。  
