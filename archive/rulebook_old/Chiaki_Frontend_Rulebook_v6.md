...
### Shared breakpoint ルール

breakpoint は app 全体で次の値だけを使います。  
近い値に寄せられる場合は必ず集約し、one-off の breakpoint は増やしません。
...
- media query は **`rem` で統一**する
- 既存の `640px` / `900px` / `959px` / `1000px` などは、構造上問題がなければ最も近い shared breakpoint に寄せる
- breakpoint は「構造切り替え」のみに使い、細かな見た目調整はまず `clamp()` と token で吸収する
- どうしても shared breakpoint では成立しない場合だけ、理由を明確にしたうえで例外を認める

#### このプロジェクトでの判断

現在の token・`clamp()`・CSS 設計を見る限り、**breakpoint は `rem` の方が適切**です。  
理由は、ルール自体が `rem` 基準で定義されていること、アクセシビリティ上もブラウザの文字サイズ変更に追従しやすいこと、そして一方で spacing / typography token は既に `px` ベースで安定しているため、**breakpoint だけ `rem`、値 token は現状どおり `px` / `clamp()`** が最も実務的だからです。
...
このファイルは `Chiaki Frontend Rulebook v6` を基準版とする。  
...
- 共有 breakpoint を `48rem / 64rem / 75rem / 90rem` に統一
- breakpoint は `rem`、token は既存どおり `px` / `clamp()` を維持する方針を明記
- 近い値の one-off breakpoint を shared breakpoint へ集約する運用ルールを追加
- Home のカードグリッド基準を shared breakpoint ベースに更新
