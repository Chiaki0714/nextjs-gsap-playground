...
## primitive / semantic token の設計

再利用性の高い primitive token と、UI の役割を表す semantic token を分けて管理します。  
component 専用の調整値まで global に持ち込まず、**複数ページで横展開しやすいものだけ** を token 化します。
...
- semantic token は floating UI、panel、indicator など役割単位で使う
- `control-*` は Back ボタンや floating control に使う
- `panel-*` は半透明 panel や soft surface に使う
- `ui-line-*` / `ui-dot-*` は indicator、progress line、pagination dot などに統合して使う
- component 専用の offset、padding、hover shift などは local custom property で持つ

---

## カラー設計の考え方

共通色は CSS 変数で管理し、OS テーマに追従させます。  
配色切り替えはコンポーネントごとではなく、`globals.css` の token 切り替えで制御します。
...
- 共有 UI の基準色は変数で管理する
- `dummy-*` token は廃止し、上下のダミー section も OS テーマ準拠の色で扱う
- メインの実験 section は `background` と同色に埋もれないよう、`surface-*` や page local の背景差で主役を作る
- 実験ページ専用のグラデーションや一時的な検証色は、そのページ限定で直書きして構わない
...
hover 表現は全端末共通の前提にしません。  
スマホやタブレットでは hover が存在しない、あるいは不自然に残留することがあるため、hover は hover 可能な入力環境に限定して適用します。

### 基本方針

- hover スタイルは `@media (hover: hover) and (pointer: fine)` の中でのみ書く
- touch 中心端末では hover を前提にしない
...
このプロジェクトでは OS テーマ（light / dark）に対応します。  
テーマ切り替えはコンポーネント単位ではなく、**global token の切り替えで制御**します。

### 基本方針

- カラーはすべて CSS 変数（design token）で管理する
- コンポーネント側でテーマ分岐を書かない
...
- `#fff` や `#000` のようなベタ書きで UI を構築しない
- コンポーネントごとに light / dark 分岐を持たせない

テーマ対応の責務は `globals.css` に集約します。
...
- `will-change` は継続的な transform / opacity アニメーションなど、効果が明確な対象に限定する
- hover 程度の軽い変化では、まず通常の transition で十分かを確認する
...
ェクトは単なる学習用サンプル置き場ではなく、今後の自分の実装基準を整理し、実案件へ横展開するための基準プロジェクトとして扱います。
...
モバイル環境では `vh` 単独を前提にしません。  
ブラウザ UI の表示状態によって viewport 高さが変動するためです。
...
- full-height section は `main` 全体ではなく、その section 自身に高さを持たせる
- `before / after` のようなダミー section も OS テーマ準拠で扱う
- モバイルでメイン section が上下の section と同化しやすい場合は、page local の背景差で境界を作る
...
一覧性を優先し、Home の幅は **page.module.css 側の `.container` で完結** させます。  
global utility に依存せず、最大幅の基準には global token の `--container-wide` を使います。
...
このファイルは `Chiaki Frontend Rulebook v4` を基準版とします。
