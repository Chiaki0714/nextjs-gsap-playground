...
意味のあるコンテンツ区画。
セクション単位の `padding-block`、背景切り替え、区切りを担当する。
見出しを伴うことを基本とする。
レイアウト都合だけの場合は `div` を使う。
...
`container` と同じ要素に付与し、内部レイアウトを担当する。
`flex` / `grid` / `gap` / columns などはすべて `content` 側に持たせる。

```tsx
<div className={clsx('containerWide', styles.content)}>
```

### container / content 運用ルール（新規）

- `container` は global class として定義する
- `content` は module.css に定義する
- 基本は同一要素に併記する
...
- container と content の責務を分離した方が構造が明確になる
- レイアウトが複雑になる

### Full-Bleed 補足（新規）

- full-bleed 要素は container の外に配置する
- container 内で無理にフル幅を作らない
- absolute による拡張は禁止
